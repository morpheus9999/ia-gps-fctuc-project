...
sys.path.append(os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), "../../sumo-0.12.2/tools/lib"))
import sumonet
...
optParser.add_option("-B", "--batch", type="int", default=1, help="batch size")
...
for depart in range(options.begin, options.end, options.period):
    for batchidx in range(options.batch):
        label = "%s%s_%s" % (options.tripprefix, idx, batchidx)
...
if options.routefile:
    subprocess.call(['../../sumo-0.12.2/bin/duarouter', '-n', options.netfile, '-t', options.tripfile, '-o', options.routefile])
